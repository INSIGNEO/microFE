% function S1(img_folder, img_names, Grey_boneThreshold, nCells, Grey_marrowThreshold)
Grey_boneThreshold = str2num(Grey_boneThreshold)
nCells = str2num(nCells)

dirOutput = dir(fullfile(img_folder, img_names));
fileNames = {dirOutput.name}';
numFrames = numel(fileNames);
I = imread(fullfile(img_folder, fileNames{1}));

Grey_Rec = zeros([size(I) numFrames],class(I));
Grey_Rec(:,:,1) = I;

for p = 2:numFrames
  Grey_Rec(:,:,p) = imread(fullfile(img_folder, fileNames{p}));
...
Grey_marrowThreshold = str2num(Grey_marrowThreshold)
